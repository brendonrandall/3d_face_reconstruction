...
for entry in os.scandir(args.input_path):
    if (entry.path.endswith(".jpg") 
        or entry.path.endswith(".png")) and entry.is_file():
            ...
            #set output vars
            calculated_output_path = os.path.join(args.output_path, dataset_name)
            if not os.path.exists(calculated_output_path):
                os.makedirs(calculated_output_path, exist_ok=True)
            output_detections_path = os.path.join(calculated_output_path, 'detections')
            ...
            shutil.copy2(input_image_path, calculated_output_path)
